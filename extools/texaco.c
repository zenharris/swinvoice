...
int linedt(edtstr,row,col,lnth,attr,pos,control,cmndline,filedef)
char *edtstr;
...
/* ZEN      Refresh(crnt_window->display_id);   */
   ...
   /*   return (strncmp(comp_str,edtstr,lnth) != 0); */
	...
	if (strlen(edtstr) > lnth) {
		strcpy(line_overflow,edtstr+(lnth));
		edtstr[lnth] = 0;
	}
	for (c = strlen(edtstr);c > 0	&& edtstr[c-1] == ' ';c--) edtstr[c-1] = 0;
...
/*      inchr = exinkey(&scan); */ 
      inchr = command_line(cmndline,&scan,&msg,filedef);
      switch (inchr) {
         case 0 : switch (scan) {
                      ...
                      case SMG$K_TRM_REMOVE :if (!read_only && !block_insdel && strlen(edtstr) > *pos) {
                                      delete(edtstr,*pos);
                                      if (line_overflow[0] == 0) {
                                         exscrprn(edtstr+*pos,row,col+*pos,attr);
                                         exscrprn(" ",row,col+strlen(edtstr),attr);
                                      } else {
                                         exchrstr(edtstr,line_overflow[0]);
                                         exscrprn(edtstr+*pos,row,col+*pos,attr);
                                         delete(line_overflow,0);
                                      }
                                   }
                                   break;
         ...
         default : if (!read_only/* && (strlen(edtstr) <= (lnth - 1)) || !insert_mode)*/) {
                      ...
                      if (block_insdel || !insert_mode) {
                         ...
                         if (strlen(edtstr) > (lnth - 1)){
                            insert(line_overflow,0,edtstr[lnth-1]);
                            edtstr[lnth -1] = 0;
                         }
                         insert(edtstr,*pos,inchr);
                         if (*pos <= lnth) exscrprn(edtstr+*pos,row,col+*pos,attr);
                      }
                      (*pos)++;
   ...
   *crnt_pos = 0;
   if (*word_warp_flag){
   	...
   	*crnt_pos = strlen((*crnt_list)->storage);
		strncat((*crnt_list)->storage,line_overflow,right_margin - *crnt_pos);
	}
}
...
void texaco (crnt_list,t_line,b_line,retcode,edited,lines_limit,right_limit,line_ed,cont,cmndline,filedef)
struct line_rec **crnt_list;
...
/*   unsigned int cont_byte = WORD_WRAP;*/
   ...
   if ((*crnt_list) != NULL){
      ...
      (*line_ed)(crnt_list,0,0,0,0,NULL,&control,filedef);
...
/*      exclreol(t_line,70,LIGHTGRAY);
      ...
      (*line_ed)((*crnt_list)->storage,0,0,right_margin,0,NULL,&COMP_LOAD,filedef);
 
     scan = (*line_ed)((*crnt_list)->storage,crnt_line,0,right_margin,WHITE,&crnt_pos,&cont,cmndline,filedef);
 
      if ((*line_ed)((*crnt_list)->storage,0,0,right_margin,0,NULL,&COMP_CHECK,filedef)) *edited = TRUE;
